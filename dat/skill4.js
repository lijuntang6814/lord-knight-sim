	...
	// ポイント未処理時
		...
		if(use1 + use2 + 2 <= jobchange1 + maxjob){
			...
			document.getElementById("msgid").innerHTML = " 1st: " + disp1
				+ "/" + jobchange1 + " �ｭ 2nd: " + disp2
				+ "/" + maxjob + " �ｭ remaining:" + eval( rest1 + rest2 );
		}if(use1 + use2 + 2 > jobchange1 + maxjob || use2 + 1 > maxjob){
			document.getElementById("msgid").style.backgroundColor = "#ffdddd";
			document.getElementById("msgid").innerHTML = " 1st: " + disp1
				+ "/" + jobchange1 + " �ｭ 2nd: " + disp2
				+ "/" + maxjob + " �ｭ over:" + eval( (use1 + use2 + 2) + (rest1 + rest2) - (jobchange1 + maxjob) );
		}
	}
	if(type == 2){
		if(use1 <= jobchange1 - 1){
		...
		}if(use3 <= 64 - subp2 - subp3){
			jobp3 = use3 + 1 + subp2 + subp3;
			rest3 = 64 - subp2 - subp3 - use3;
		}if(use3 > 64 - subp2 - subp3){
			jobp3 = 65;
			rest3 = 0;
		}
		...
		disp3 = jobp3;
			if(subp2 != 0 || subp3 != 0 || use3 >= 65) disp3 = jobp3 + "<span style='font-size:10px;'> (" + subp2 + "+" + subp3 + "+" + use3 + ") </span>";

	// ポイント未処理時
		if(use1 +1 < jobchange1) disp1 = "<span style='color:#aa0033;'>" + disp1 + "</span>";
		if(use2 + 1 < jobchange2 - subp1) disp2 = "<span style='color:#aa0033;'>" + disp2 + "</span>";
		if(use3 + 1 < 65 - subp2 - subp3) disp3 = "<span style='color:#aa0033;'>" + disp3 + "</span>";

		if(use1 + use2 + use3 + 3 <= jobchange1 + jobchange2 + 65 && use3 < 65){
			document.getElementById("msgid").style.backgroundColor = "#ffffff";
			document.getElementById("msgid").innerHTML = " 1st: " + disp1
				+ "/" + jobchange1 + " �ｭ 2nd: " + disp2
				+ "/" + jobchange2 + " �ｭ 3rd: " + disp3
				+ "/65 �ｭ remaining:" + eval( (rest1 + rest2 +rest3) );
		}if(use1 + use2 + use3 + 3 > jobchange1 + jobchange2 + 65 && use3 < 65){
			document.getElementById("msgid").style.backgroundColor = "#ffdddd";
			document.getElementById("msgid").innerHTML = " 1st: " + disp1
				+ "/" + jobchange1 + " �ｭ 2nd: " + disp2
				+ "/" + jobchange2 + " �ｭ 3rd: " + disp3
				+ "/65 �ｭ over:" + eval( (use1 + use2 + use3 + 3) + (rest1 + rest2 +rest3) - (jobchange1 + jobchange2 + 65) );
		}if(use2 + use3 + 2 > jobchange2 + 65){
			document.getElementById("msgid").style.backgroundColor = "#ffdddd";
			document.getElementById("msgid").innerHTML = " 1st: " + disp1
				+ "/" + jobchange1 + " �ｭ 2nd: " + disp2
				+ "/" + jobchange2 + " �ｭ 3rd: " + disp3
...
function codeCopy(){
	tmp2 = "https://skillsim.irowiki.org/"+job+".html?"+Save("scode");
